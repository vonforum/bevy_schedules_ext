...
pub trait WorldExt {
	...
	fn init_schedule_container<S: Default + Send + Sync + 'static>(&mut self, label: InternedScheduleLabel);
	/// Initializes the [`ScheduleContainers`] resource and inserts a container for the given label, if not yet present.
	fn insert_schedule_container<S: Default + Send + Sync + 'static>(&mut self, label: InternedScheduleLabel, container: S) -> bool;
	...
	fn insert_schedule_container_system_marker<S: Default + Send + Sync + 'static>(&mut self, label: InternedScheduleLabel) -> bool;
}
